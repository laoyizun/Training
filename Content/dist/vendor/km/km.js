...
define('km/window', ['jquery', 'km/dragdrop', 'km/popTips', 'km/loading', 'km/util'], function ($, DragDrop, popTips, Loading, util) {
    ...
    Window.prototype.open = function () {
        ...
        $wins.each(function () {
            var $el = $(this),
                obj = $.data(this, 'window');

            if (obj && obj.identity != self.identity && obj.isClose&&obj.options.canDestory) {
                obj.destory();
                obj = null;
            }
        })
        ...
        $.when(
            ...
            if (self.options.iframe) {
                ...
                url += "&popWinId=" + (self.options.id ? self.options.id : self.identity);
    ...
    Window.prototype.destory = function () {
        ...
        this.$backdrop.off('click.window').remove();
        this.$win.off().remove();
        this.$window.off('resize.window.' + this.identity);
        this.$backdrop = null;
        this.$win = null;
        this.$window = null;
        this.$document = null;
    ...
    */
    ...
    Window.alert = function (title, content, onOk) {
        ...
        if (!win) {
            win = new Window({ width: 400, backdropClose: false, zIndex: 9999,canDestory:false });
     ...
     * @return {Void}    
     */
    Window.confirm = function (title, content, onOk, onClose,btns) {
        if ($.isFunction(content)) {
            onClose = onOk;
            onOk = content;
            content = title;
